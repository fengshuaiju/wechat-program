      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          if (money <= 0) {
            // 直接使用余额支付
            wx.request({
              url: app.globalData.url + app.globalData.subDomain + '/order/pay',
              method: 'POST',
              header: {
              ...
              success: function (res2) {
                wx.redirectTo({
                  url: "/pages/order-list/index?currentType=1&share=1"
                });
              }
            })
          } else {
            wxpay.wxpay(app, money, orderId, "/pages/order-list/index?currentType=1&share=1");
          }
        } else {
          wx.showModal({
            title: '错误',
